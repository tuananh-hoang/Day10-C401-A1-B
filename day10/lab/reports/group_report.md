...
|-----|------------------|-----------------|
| Person 1 | Cleaning Owner + Monitoring Owner (kết hợp) | 3 cleaning rules + 3 monitoring functions + test scripts |
| Person 2 | Quality Owner + Evaluation Owner (kết hợp) | 4 expectations + eval retrieval + quality report |
| Person 3 | Documentation Owner + Embed Owner (kết hợp) | 3 docs + group report + idempotency |

**Ghi chú phân vai:** Nhóm 3 người nên kết hợp vai trò để cover đủ 4 vai trong README (Ingestion/Cleaning/Embed/Monitoring-Docs). Person 1 đảm nhận Cleaning + Monitoring, Person 2 đảm nhận Quality + Eval, Person 3 đảm nhận Docs + Embed/Ingestion.
...
### Run IDs quan trọng
- **`baseline-person1`**: Baseline trước khi thêm rules mới (6 cleaned, 4 quarantine)
- **`clean-run`**: Sau khi thêm 3 rules của Person 1 (5 cleaned, 5 quarantine)
- **`inject-bad`**: Inject corruption test (6 cleaned, 4 quarantine)
...
| Rule / Expectation mới | Trước (baseline) | Sau (clean-run) | Chứng cứ |
|------------------------|------------------|-----------------|----------|
| **Rule 7: Quarantine metadata comments** | cleaned=6, quarantine=4 | cleaned=5, quarantine=5 (+1) | Row 3 quarantined: `contains_metadata_comments` |
| **Rule 8: Normalize em dash** | Text có "—" và "–" | Text có "-" (normalized) | `test_rules_impact.py` row 3 |
| **Rule 9: Detect conflicting days** | No detection | Quarantine row 4 in test data | `test_dirty_for_rules.csv` row 4 |
| **E7: No BOM/control chars** | Not checked | PASS (BOM stripped) | `quality/expectations.py` |
| **E8: Chunk IDs unique** | Not checked | PASS (no duplicates) | `quality/expectations.py` |
| **E9: No metadata in cleaned** | Not checked | PASS (Rule 7 works) | Cross-validation with Rule 7 |
| **E10: Doc ID in allowlist** | Not checked | PASS (all docs valid) | Synced with `ALLOWED_DOC_IDS` |
...
**Person 2's new expectations (4 expectations):**
...
| Metric | Baseline | Clean-run | Inject-bad | Change |
|--------|----------|-----------|------------|--------|
| Raw records | 10 | 10 | 10 | 0 |
| Cleaned | 6 | 5 | 6 | -1 (better quality) |
| Quarantine | 4 | 5 | 4 | +1 (more aggressive filtering) |
| Quarantine rate | 40% | 50% | 40% | +10% (expected) |

**Tại sao cleaned giảm là TỐT**:
- Cleaned giảm = loại bỏ dữ liệu xấu = GOOD
- Rule 7 bắt được metadata mà baseline bỏ sót
- Chatbot chỉ nhận dữ liệu sạch hơn
...
### 4b. Monitoring enhancements (Person 1 - BONUS +1)
...
- **Defense-in-depth**: Rule 7 (metadata) chạy trước refund fix → nhiều lớp bảo vệ
- **Cross-validation**: E9 validate output của Rule 7 → đảm bảo cleaning rules hoạt động
- **Test coverage**: 3 test scripts (rules impact, inject corruption, monitoring) với output rõ ràng
...
**Tổng số từ**: ~950 từ (trong khuyến nghị 600-1000 từ)

**Prepared by**: Team C401-A1-B  
**Date**: 2026-04-15  
